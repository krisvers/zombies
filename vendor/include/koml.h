...
static inline int koml_table_alloc_new(koml_table_t* table) {
	++table->length;
	if (table->hashes == NULL) {
		table->hashes = (unsigned long long int*) malloc(table->length * sizeof(unsigned long long int));
	} else {
		table->hashes = (unsigned long long int*) realloc((void*) table->hashes, table->length * sizeof(unsigned long long int));
	}
	...
	if (table->symbols == NULL) {
		table->symbols = (koml_symbol_t*) malloc(table->length * sizeof(koml_symbol_t));
	} else {
		table->symbols = (koml_symbol_t*) realloc(table->symbols, table->length * sizeof(koml_symbol_t));
	}
...
static inline int koml_array_alloc_new(koml_array_t* array) {
	++array->length;
	if (array->strides == NULL) {
		array->strides = (unsigned long long int*) malloc(array->length * sizeof(unsigned long long int));
	} else {
		array->strides = (unsigned long long int*) realloc((void*) array->strides, array->length * sizeof(unsigned long long int));
	}
...
static inline int koml_array_alloc_new_amount(koml_array_t* array, unsigned long long int amount) {
	array->length = amount;
	if (array->strides == NULL) {
		array->strides = (unsigned long long int*) malloc(array->length * sizeof(unsigned long long int));
	} else {
...
static inline char koml_internal_is_whitespace(char c) {
	if (c == '\t' || c == '\r' || c == '\n' || c == ' ') {
		return 1;
	}
...
static inline unsigned char koml_internal_is_boolean(char* start, unsigned long long int length) {
	if (length == 1) {
...
static inline unsigned char koml_internal_is_num(char c) {
...
static inline void koml_printline(char* buffer, unsigned long long int line, unsigned long long int column) {
	while (*buffer != '\0') {
		if (*buffer == '\n') {
			--line;
		}
...
static inline void koml_printcursor(unsigned long long int column) {
	while (--column) {
		putc(' ', stdout);
	}
	putc('^', stdout);
...
static inline const char* koml_type_strings[] = {
	"unknown",
	"int",
	"float",
	"string",
...
static inline void koml_array_print(koml_array_t* array) {
...
int koml_table_load(koml_table_t* out_table, char* buffer, unsigned long long int buffer_length) {
	...
	for (unsigned long long int i = 0; i < buffer_length; ++i) {
		...
		case KOML_PARSER_STATE_SECTION_NAME:
			...
			} else if ((isalnum(c) || ispunct(c)) && word.start != NULL && koml_internal_is_whitespace(prevc)) {
		...
		case KOML_PARSER_STATE_INTEGER_WAIT:
			if (koml_internal_is_whitespace(c)) {
				continue;
			}
		...
		case KOML_PARSER_STATE_INTEGER_NAME:
			...
			if (c == '=') {
				...
				if (section.start != NULL) {
					...
					out_table->symbols[out_table->length - 1].name = (char*) malloc(tmp_length + 1);
		...
		case KOML_PARSER_STATE_INTEGER_VALUE:
			...
			if (c == ';') {
				int value = koml_internal_wtoi(word.start, word.length, 10);
		...
		case KOML_PARSER_STATE_FLOAT_VALUE:
			...
			if (c == ';') {
				float value = koml_internal_wtof(word.start, word.length);
		...
		case KOML_PARSER_STATE_STRING_VALUE:
			...
			if (c == '"') {
				...
				out_table->symbols[out_table->length - 1].data.string = (char*) malloc(word.length + 1);
				...
				out_table->symbols[out_table->length - 1].data.string = (char*) malloc(ptr->stride + 1);
				if (out_table->symbols[out_table->length - 1].data.string == NULL) {
					printf("Internal error (line %llu: column %llu)\n  | ", line + 1, column + 1);
					koml_printline(buffer, line, column);
					printf("\n  | ");
		...
		case KOML_PARSER_STATE_BOOLEAN_VALUE:
			...
			if (c == ';') {
				if (!koml_internal_is_boolean(word.start, word.length)) {
				...
				unsigned char value = koml_internal_wtotf(word.start, word.length);
		...
		case KOML_PARSER_STATE_ARRAY_TYPE_WAIT:
			...
			if (state == KOML_PARSER_STATE_ARRAY_TYPE_WAIT && !koml_internal_is_whitespace(c)) {
		...
		case KOML_PARSER_STATE_ARRAY_VALUE:
			...
			if (c == '@') {
				...
				case KOML_TYPE_STRING:
					...
					memcpy(out_table->symbols[out_table->length - 1].data.array.strides, ptr->data.array.strides, ptr->data.array.length * 4);
					for (unsigned long long int i = 0; i < ptr->data.array.length; ++i) {
						out_table->symbols[out_table->length - 1].data.array.elements.string[i] = (char*) malloc(ptr->data.array.strides[i] + 1);
			...
			case KOML_TYPE_INT:
				if (c == ',') {
					...
					state = KOML_PARSER_STATE_NONE;
				} else if (!koml_internal_is_num(c)) {
					printf("Invalid integer value (line %llu: column %llu)\n  | ", line + 1, column + 1);
					koml_printline(buffer, line, column);
					printf("\n  | ");
					koml_printcursor(column);
			...
			case KOML_TYPE_FLOAT:
				if (c == ',') {
					...
					state = KOML_PARSER_STATE_NONE;
				} else if (!koml_internal_is_num(c) && c != '.') {
					printf("Invalid float value (line %llu: column %llu)\n  | ", line + 1, column + 1);
					koml_printline(buffer, line, column);
					printf("\n  | ");
					koml_printcursor(column);
			...
			case KOML_TYPE_STRING:
				...
				if (c == '"') {
					...
					out_table->symbols[out_table->length - 1].data.array.elements.string[out_table->symbols[out_table->length - 1].data.array.length - 1] = (char*) malloc(word.length + 1);
		...
		if (state == KOML_PARSER_STATE_NONE && !koml_internal_is_whitespace(c)) {
